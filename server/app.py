...
def _students_table(campus_id=None):
    """罗泾校区使用独立 students_luojing 表，其他校区用 students 表"""
    if campus_id == 'luojing':
        return 'students_luojing'
    return 'students'

def _teacher_roster_table(campus_id=None):
    """罗泾校区使用独立 teacher_roster_luojing 表，其他校区用 teacher_roster 表"""
    if campus_id == 'luojing':
...
DISABLE_DEMO = os.environ.get('DISABLE_DEMO', '').lower() in ('1', 'true', 'yes', 'on')
...
def _ensure_user_bindings_table():
    ...
    for col_def in [('sub_role','TEXT'),('bound_grade','TEXT'),('bound_class','TEXT')]:
...
def _current_user():
    ...
    if auth.startswith('Bearer '):
        ...
        if info and info.get('unionId'):
            ...
            # 从 parent_kids 取激活的孩子
            db = get_db()
            pk = db.execute('SELECT kid_id_card, kid_name, kid_grade, kid_class, campus FROM parent_kids WHERE union_id = ? AND is_active = 1 LIMIT 1', (uid,)).fetchone()
            
            if b:
                base_role = b['role']
                # 双角色检测：教师也有绑定的孩子 → 可切家长模式
                if base_role == 'teacher':
                    all_kids = db.execute('SELECT kid_id_card, kid_name, kid_grade, kid_class, campus, is_active FROM parent_kids WHERE union_id = ?', (uid,)).fetchall()
                    if all_kids:
                        ...
                        if override == 'parent':
                            bound = next((k for k in all_kids if k['is_active']), all_kids[0])
                            return {
                                'role': 'parent',
                                'sub_role': None,
                                'bound_id_card': bound['kid_id_card'],
                                'bound_grade': bound['kid_grade'],
                                'bound_class': bound['kid_class'],
                                'identity': f"dt:{uid}",
                            }
                return {
                    'role': base_role,
                    'sub_role': b.get('sub_role'),
                    'bound_id_card': pk['kid_id_card'] if pk else None,
                    'bound_grade': pk['kid_grade'] if pk else None,
                    'bound_class': pk['kid_class'] if pk else None,
                    'identity': f"dt:{uid}",
                }
...
def auth_me():
    ...
    if u['bound_id_card']:
        ...
        campus = _resolve_campus()
        stable = _students_table(campus)
        cur = db.execute(f'SELECT id_card, name, grade_name, class_name FROM {stable} WHERE id_card = ?', (u['bound_id_card'],))
        row = cur.fetchone()
        if row:
            info['boundStudent'] = {
                'idCard': row['id_card'],
                'displayName': mask_name(row['name']),
    ...
    if u['role'] == 'parent' and 'identity' in u and u['identity'].startswith('dt:'):
        uid = u['identity'][3:]
        db = get_db()
        kids = db.execute('SELECT kid_id_card, kid_name, kid_grade, kid_class, campus, is_active FROM parent_kids WHERE union_id = ? ORDER BY bound_at', (uid,)).fetchall()
        if kids:
            info['kids'] = [{'idCard': k['kid_id_card'], 'name': k['kid_name'], 'grade': k['kid_grade'], 'class': k['kid_class'], 'campus': k['campus'], 'active': bool(k['is_active'])} for k in kids]
...
def switch_role():
    ...
    av = session.get('available_roles')
    if not av:
        return jsonify({'success': False, 'error': '当前用户无双角色权限'}), 403
...
def dingtalk_login():
    ...
    支持 campus 参数：POST body 中传 campus=benbu|baolin|luojing，使用该校区专属 AppKey/Secret。"""
    payload = request.get_json() or {}
    auth_code = (payload.get('authCode') or '').strip()
    campus_id = (payload.get('campus') or '').strip()
    # 彻底清洗：去掉所有不可打印字符，只保留字母数字
    import re
    auth_code_clean = re.sub(r'[^a-zA-Z0-9]', '', auth_code)
    app.logger.info(f'[钉钉登录] 收到请求 campus={campus_id or "benbu"} raw_len={len(auth_code)} clean_len={len(auth_code_clean)} raw_first={repr(auth_code[:40])} clean_first={repr(auth_code_clean[:40])}')
    
    # 校区专属或全局配置（未指定 campus 时默认本部）
    if not campus_id:
        campus_id = 'benbu'
    ...
    if not (app_key and app_secret):
        return jsonify({'error': f'钉钉未配置 (campus={campus_id or "default"})', 'configured': False}), 503
    if not auth_code_clean:
        return jsonify({'error': 'authCode required'}), 400
    try:
        # dd.getAuthCode 返回的 authCode 先走免登兑码
        user = _exchange_auth_code_via_accesstoken(app_key, app_secret, auth_code_clean)
        if not user or not (user.get('unionId') or user.get('userId')):
            app.logger.info('[钉钉登录] 免登兑码失败，尝试 sns/getuserinfo_bycode')
            user = _exchange_auth_code_old(app_key, app_secret, auth_code_clean)
        if not user or not (user.get('unionId') or user.get('userId')):
            app.logger.info('[钉钉登录] 旧版 API 失败，尝试新版 v1.0/oauth2/userAccessToken')
            user = _exchange_auth_code_new(app_key, app_secret, auth_code_clean)
        if not user:
            return jsonify({'error': '钉钉身份校验失败：三个 API 均无法获取用户信息'}), 502
        ...
        try:
            ...
            if not existing and union_id:
                ...
                db.execute('INSERT INTO user_bindings(dingtalk_unionid, role, bound_name, dingtalk_userid) VALUES(?,?,?,?)',
                           (union_id, default_role, nick, dingtalk_userid))
                db.commit()
                teacher_info = f' teacher_campus={campus_id}' if teacher_row else ''
                app.logger.info(f'[钉钉登录] 新用户自动入库: {union_id} ({default_role}) {nick}{teacher_info}')
            ...
            app.logger.warning(f'[钉钉登录] upsert user_bindings 失败: {ee}')
        sess_token = _sign_session({'unionId': union_id, 'nick': nick, 'campus': campus_id, 'ts': int(time.time())})
...
def stats_class_summary():
    ...
    base_sql = '''SELECT grade_name, class_name, week_number, parity, weekday, choice, COUNT(*) AS cnt
                  FROM meal_choices WHERE 1=1'''
    params = []
    if grade:
        base_sql += ' AND grade_name = ?'
        params.append(grade)
    if week:
        base_sql += ' AND week_number = ?'
        params.append(week)
    if parity:
        base_sql += ' AND parity = ?'
        params.append(parity)
    base_sql += ' GROUP BY grade_name, class_name, week_number, parity, weekday, choice'
    base_sql += ' ORDER BY grade_name, class_name, week_number, parity, weekday'
...
def stats_grade():
    ...
    db = get_db()
    week = request.args.get('week', type=int)
    where = 'WHERE week_number = ?' if week else ''
    params = (week,) if week else ()
    cur = db.execute(f'''
        SELECT grade_name, week_number, parity, weekday, choice, COUNT(*) AS cnt
        FROM meal_choices
        {where}
        GROUP BY grade_name, week_number, parity, weekday, choice
        ORDER BY grade_name, week_number, parity, weekday
    ''', params)
    out = {}
    for r in cur.fetchall():
        key = (r['grade_name'], r['week_number'], r['parity'])
        out.setdefault(key, {})
        out[key][f"{r['weekday']}{r['choice']}"] = r['cnt']
    rows = [{'gradeName': k[0], 'week': k[1], 'parity': k[2], **v} for k, v in out.items()]
    return jsonify({'rows': rows})
...
def export_school_meal():
    ...
    # ─ Sheet 1: 各年级人数 ─
    ws = wb.create_sheet('各年级人数')
    _styled_header(ws, ['年级','人数'])
    rows = db.execute('SELECT grade_name, COUNT(*) AS cnt FROM students GROUP BY grade_name ORDER BY grade_name').fetchall()
    total = 0
    for i, r in enumerate(rows, 2):
        ws.cell(row=i, column=1, value=r['grade_name'] or '-')
        ws.cell(row=i, column=2, value=r['cnt'])
        total += r['cnt']
    ws.cell(row=len(rows)+2, column=1, value='合计'); ws.cell(row=len(rows)+2, column=2, value=total)
    _auto_width(ws)
    ...
    for parity_key, parity_label in (('odd','奇数周'),('even','偶数周')):
        ...
        params = [parity_key]
        if week:
            where += ' AND week_number=?'; params.append(week)
        cur = db.execute(f'''
            SELECT grade_name, weekday, choice, COUNT(*) AS cnt
            FROM meal_choices {where}
            GROUP BY grade_name, weekday, choice ORDER BY grade_name, weekday''', params)
        agg = {}
        for r in cur:
            g = r['grade_name'] or '-'
            agg.setdefault(g, {})[f"{r['weekday']}{r['choice']}"] = r['cnt']
    ...
    # ─ Sheet 4: 班级粒度明细 ─
    ws = wb.create_sheet('班级粒度')
    headers = ['年级','班级','周次','奇/偶'] + [f'{d}{c}' for d in DAY for c in ('A','B')] + ['总 A','总 B']
    _styled_header(ws, headers)
    where = ''
    params = []
    if week:
        where = 'WHERE week_number=?'; params = [week]
    cur = db.execute(f'''SELECT grade_name, class_name, week_number, parity, weekday, choice, COUNT(*) AS cnt
                         FROM meal_choices {where}
                         GROUP BY grade_name, class_name, week_number, parity, weekday, choice
                         ORDER BY grade_name, class_name, week_number, parity, weekday''', params)
    agg = {}
    for r in cur:
        key = (r['grade_name'] or '-', r['class_name'] or '-', r['week_number'], r['parity'])
        agg.setdefault(key, {})[f"{r['weekday']}{r['choice']}"] = r['cnt']
    for r_idx, (key, m) in enumerate(sorted(agg.items()), 2):
        ws.cell(row=r_idx, column=1, value=key[0])
        ws.cell(row=r_idx, column=2, value=key[1])
...
<ol>
  ...
  <li>在「开发管理」中设置：<br>
      ...
      • 服务器出口IP：填写本服务器IP (118.25.196.70)</li>
...
def dingtalk_awards_page():
    """钉钉奖状分发页面。根据 campus 参数注入校区专属配置。"""
    campus_id = request.args.get('campus', '').strip()
    if not campus_id:
        campus_id = 'benbu'  # 默认本部
    dt_cfg = _get_campus_dt(campus_id)
    cfg = _load_campus_config()
    campus_info = cfg.get(campus_id, {})
    ...
    # 给 bootstrap.js 加版本号，防止 WebView 缓存旧 JS
...
def campus_info():
    ...
    if school_name:
        if campus_id == 'luojing':
            rows = db.execute(
                f'SELECT DISTINCT grade_name FROM {stable} ORDER BY grade_name'
            ).fetchall()
        else:
            rows = db.execute(
                f'SELECT DISTINCT grade_name FROM {stable} WHERE school_name = ? ORDER BY grade_name',
                (school_name,)
            ).fetchall()
...
def campus_students():
    ...
    if where:
        sql += ' WHERE ' + ' AND '.join(where)
    sql += ' ORDER BY grade_name, class_name, name LIMIT 50'
    rows = db.execute(sql, params).fetchall()
    students = []
    for r in rows:
        students.append({
...
def auth_bind_child():
    ...
    if existing:
        ...
        db.execute('UPDATE parent_kids SET is_active = 0 WHERE union_id = ?', (union_id,))
        db.execute('''INSERT INTO parent_kids(union_id, kid_id_card, kid_name, kid_grade, kid_class, campus, is_active)
            VALUES(?,?,?,?,?,?,1)''',
            (union_id, student['id_card'], student['name'], student['grade_name'], student['class_name'], campus_id))
    db.commit()
    
    all_kids = db.execute('SELECT kid_id_card, kid_name, kid_grade, kid_class, campus, is_active FROM parent_kids WHERE union_id = ? ORDER BY bound_at', (union_id,)).fetchall()
    kids_list = [{'idCard': k['kid_id_card'], 'name': k['kid_name'], 'grade': k['kid_grade'], 'class': k['kid_class'], 'campus': k['campus'], 'active': bool(k['is_active'])} for k in all_kids]
...
def auth_binding_status():
    ...
    if av:
        ...
        if u['role'] == 'parent':
            ...
            kids = db.execute('SELECT kid_id_card, kid_name, kid_grade, kid_class, campus, is_active FROM parent_kids WHERE union_id = ? ORDER BY bound_at', (uid,)).fetchall()
            if kids:
                resp['kids'] = [{'idCard': k['kid_id_card'], 'name': k['kid_name'], 'grade': k['kid_grade'], 'class': k['kid_class'], 'campus': k['campus'], 'active': bool(k['is_active'])} for k in kids]
...
def auth_switch_kid():
    ...
    db = get_db()
    # 验证绑定关系存在
    kb = db.execute('SELECT id FROM parent_kids WHERE union_id = ? AND kid_id_card = ?', (union_id, kid_id_card)).fetchone()
    if not kb:
        return jsonify({'error': '未找到该孩子的绑定记录'}), 404
    # 切换激活
    db.execute('UPDATE parent_kids SET is_active = 0 WHERE union_id = ?', (union_id,))
    db.execute('UPDATE parent_kids SET is_active = 1 WHERE union_id = ? AND kid_id_card = ?', (union_id, kid_id_card))
    db.commit()
    kid = db.execute('SELECT kid_name, kid_grade, kid_class FROM parent_kids WHERE union_id = ? AND kid_id_card = ?', (union_id, kid_id_card)).fetchone()
    return jsonify({
        'success': True,
        'kid': {'idCard': kid_id_card, 'name': kid['kid_name'], 'grade': kid['kid_grade'], 'class': kid['kid_class']},
    })
...
def auth_unbind_child():
    ...
    # 返回剩余孩子
    remaining = db.execute('SELECT kid_id_card, kid_name, kid_grade, kid_class, campus, is_active FROM parent_kids WHERE union_id = ? ORDER BY bound_at', (union_id,)).fetchall()
    return jsonify({
        'success': True,
        'message': f'已解绑: {kid_name}',
        'kids': [{'idCard': k['kid_id_card'], 'name': k['kid_name'], 'grade': k['kid_grade'], 'class': k['kid_class'], 'campus': k['campus'], 'active': bool(k['is_active'])} for k in remaining],
    })
