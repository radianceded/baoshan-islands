...
class MealChoiceAuthorizationTests(unittest.TestCase):
    ...
    def test_meal_statistics_require_teacher_and_aggregate_choices(self):
        ...
        grade_payload = grade.get_json()
        self.assertEqual(grade_payload["participantCount"], 2)
        self.assertEqual(len(grade_payload["school"]), 1)
        self.assertEqual(len(grade_payload["classes"]), 1)
        self.assertEqual(grade_payload["school"][0]["1A"], 1)
        self.assertEqual(grade_payload["school"][0]["1B"], 1)
        grade_row = grade_payload["rows"][0]
    ...
    def test_meal_reports_use_current_roster_and_ignore_unknown_demo_rows(self):
        import openpyxl
        ...
        conn.execute(
            """UPDATE students_benbu
               SET grade_name='三年级', class_name='5班'
               WHERE id_card='BS001'"""
        )
        conn.execute(
            """INSERT INTO meal_choices(
                   ...
                   parity, weekday, choice, chosen_by
               ) VALUES('DEMO-X', '测试学生', 'demo', 'demo', 17,
                        'odd', 1, 'B', 'demo')"""
        )
        conn.commit()
        conn.close()

        general_headers = self._teacher_headers(sub_role="general")
        grade = self.client.get(
            "/api/meal-stats/grade?week=17", headers=general_headers
        ).get_json()
        self.assertEqual(grade["participantCount"], 1)
        self.assertEqual({row["gradeName"] for row in grade["rows"]}, {"三年级"})
        self.assertEqual(
            {(row["grade"], row["class"]) for row in grade["classes"]},
            {("三年级", "5班")},
        )

        class_stats = self.client.get(
            "/api/meal-stats/class?week_odd=17&week_even=18",
            headers=self._teacher_headers(
                sub_role="class", grade="三年级", klass="5班"
            ),
        ).get_json()
        self.assertEqual(class_stats["summary"]["studentCount"], 1)
        self.assertEqual(class_stats["summary"]["aCount"], 5)
        self.assertEqual(class_stats["summary"]["bCount"], 5)

        school_export = self.client.get(
            "/api/export/school-meal.xlsx?week_odd=17&week_even=18",
            headers=general_headers,
        )
        self.assertEqual(school_export.status_code, 200)
        workbook = openpyxl.load_workbook(io.BytesIO(school_export.data))
        class_sheet = workbook["班级汇总"]
        classes = {
            (class_sheet.cell(row, 1).value, class_sheet.cell(row, 2).value)
            for row in range(2, class_sheet.max_row + 1)
        }
        self.assertEqual(classes, {("三年级", "5班")})

    def test_teacher_exports_use_authenticated_scope_and_workdays_only(self):
        import openpyxl
        ...
        class_export = self.client.get(
            "/api/export/class-meal.xlsx?grade=五年级&class=1班&week_odd=17&week_even=18",
            headers=self._teacher_headers(
                sub_role="class", grade="五年级", klass="1班"
            ),
        )
        ...
        school_book = openpyxl.load_workbook(io.BytesIO(school.data))
        self.assertEqual(school_book.sheetnames, ["全校汇总", "年级汇总", "班级汇总"])
        for sheet in school_book:
            headers = [cell.value for cell in sheet[1]]
            self.assertNotIn("周六A", headers)
            self.assertNotIn("周日A", headers)
        class_book = openpyxl.load_workbook(io.BytesIO(class_export.data))
        self.assertEqual(class_book.sheetnames, ["每日汇总", "学生选餐明细"])
        summary = class_book["每日汇总"]
        summary_headers = [cell.value for cell in summary[1]]
        self.assertEqual(summary_headers[2:6], ["周一A餐", "周一B餐", "周二A餐", "周二B餐"])
        self.assertEqual(summary.cell(2, 1).value, 17)
        self.assertEqual(summary.cell(2, 3).value, 2)
        self.assertEqual(summary.cell(2, 4).value, 0)
        self.assertEqual(summary.cell(3, 1).value, 18)
        self.assertEqual(summary.cell(3, 3).value, 0)
        self.assertEqual(summary.cell(3, 4).value, 2)
        class_headers = [cell.value for cell in class_book["学生选餐明细"][1]]
        self.assertNotIn("奇数周·周六", class_headers)
        self.assertNotIn("偶数周·周日", class_headers)
        detail = class_book["学生选餐明细"]
        exported_names = {
            detail.cell(row, 2).value
            for row in range(2, detail.max_row + 1)
        }
        self.assertEqual(exported_names, {"学生甲", "学生乙"})
        self.assertFalse(any("*" in name for name in exported_names))

        general_class_export = self.client.get(
            "/api/export/class-meal.xlsx?grade=五年级&class=1班&week_odd=17&week_even=18",
            headers=self._teacher_headers(sub_role="general"),
        )
        general_book = openpyxl.load_workbook(io.BytesIO(general_class_export.data))
        general_detail = general_book["学生选餐明细"]
        general_names = {
            general_detail.cell(row, 2).value
            for row in range(2, general_detail.max_row + 1)
        }
        self.assertTrue(all("*" in name for name in general_names))
    ...
    def test_menu_week_rejects_mismatched_parity(self):
        response = self.client.post(
            "/api/menus/upload",
            json={
                "week": 2,
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json()["expectedParity"], "even")
        self.assertIn("偶数周", response.get_json()["error"])
